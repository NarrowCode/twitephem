...
if api:
    ...
    if(sel == "d" or sel == "dh"):
        start_date = str(raw_input("Please specify a start date in the form [DD-MM-YYYY]. " \
                                   "Tweets after this date will be deleted: "))
        start_date_obj = datetime.strptime(start_date, DATE_FORMAT)
        end_date = str(raw_input("Please specify a end date in the form [DD-MM-YYYY]. " \
                                 "Tweets after this date will not be deleted. " \
                                 "Type [now] to delete everything up to the current day: "))
        if (end_date == "now"):
            end_date_obj = datetime.now() 
        else:
            end_date_obj = datetime.strptime(end_date, DATE_FORMAT)

        print("If you continue, every tweet between {0} and {1} will be deleted.".format(
